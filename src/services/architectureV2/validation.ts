import type { LegacyAuthorityState, QdbV2Envelope, QdbV2ResourceMetadata, RejectionCode, V2EntityCreate } from './types';

export type ValidationResult = { ok: true } | { ok: false; code: RejectionCode; detail: string };

export type IdentityValidator = {
  validatePublisher: (metadata: QdbV2ResourceMetadata, claimedPublisher: string) => ValidationResult;
...
export const validateEnvelope = <T>(envelope: QdbV2Envelope<T>): ValidationResult => {
  if (envelope.schema !== 'qdb-v2' || envelope.schemaVersion !== 2 || !envelope.recordId || !envelope.targetId || !envelope.recordType) return { ok: false, code: 'MALFORMED_ENVELOPE', detail: 'invalid qdb-v2 envelope' };
  return { ok: true };
};

export const validateEntityCreate = (metadata: QdbV2ResourceMetadata, envelope: QdbV2Envelope<V2EntityCreate>, identity: IdentityValidator): ValidationResult => {
  const checks = [validateMetadata(metadata), validateEnvelope(envelope)];
  const failed = checks.find((check) => !check.ok);
  if (failed && !failed.ok) return failed;
  if (envelope.kind !== 'entity-create' || envelope.targetId !== envelope.body.entityId || envelope.body.entityType !== envelope.recordType) return { ok: false, code: 'MALFORMED_ENVELOPE', detail: 'entity envelope target mismatch' };
