...
const order = (metadata: QdbV2ResourceMetadata) =>
...
const sameEntityCreate = (left: V2EntityCreate, right: V2EntityCreate) => {
  ...
  if (left.entityType === 'thread' && right.entityType === 'thread')
    return (
      left.parentTopicId === right.parentTopicId &&
...
export const reduceV2Creates = (
  ...
  const sorted = [...records].sort((a, b) =>
    order(a.metadata).localeCompare(order(b.metadata))
  );
  for (const record of sorted) {
    const valid = validateEntityCreate(
      record.metadata,
      record.envelope,
      identity
    );
...
export const applyOwnerEdit = (
  ...
  if (entity.entityType !== edit.targetType)
    return reject(
      ...
      'MALFORMED_ENVELOPE',
      edit.targetId,
      'owner edit target type mismatch'
    );
  ...
  if (
    edit.publisherName.trim().toLowerCase() !==
    ...
    return reject(
      ...
      'UNAUTHORIZED_PUBLISHER',
      edit.targetId,
      'owner edit publisher does not match authoritative owner'
    );
  ...
  if (!fields.ok)
    return reject(
      ...
      'FORBIDDEN_FIELD',
      edit.targetId,
      `forbidden fields: ${fields.forbidden.join(', ')}`
    );
  return {
    ...state,
    entities: {
      ...state.entities,
      [edit.targetId]: { ...entity, ...edit.changes } as V2EntityCreate,
    },
  };
};

export const legacyCanMutate = (entity: LegacyNormalizedEntity) =>
  entity.authorityState === 'APPROVED';

export const authorizeLegacyMutation = (
