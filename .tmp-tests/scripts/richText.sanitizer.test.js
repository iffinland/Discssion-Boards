...
const run = () => {
    ...
    expect(escaped.includes('&lt;script&gt;'), 'Raw script tags must be escaped into plain text.');
    ...
    expect(qdnImage.includes('data-qdn-image-placeholder="true"'), 'QDN image tag should render a lazy placeholder.');
    expect(!qdnImage.includes('<img'), 'QDN image tag must not render an image before viewport resolution.');
    ...
    const videoHtml = toRichTextHtml('[videoqdn]qdn|Qortal-Video-Bridge|qvb-example|Bridge%20video[/videoqdn]');
    expect(videoHtml.includes('data-video-embed="true"'), 'Video tag should render lightweight placeholder.');
    expect(!videoHtml.includes('<video'), 'Video tag must not render a video element during post rendering.');
};
run();
