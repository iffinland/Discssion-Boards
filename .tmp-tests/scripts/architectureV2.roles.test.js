import { buildRoleOperationEnvelope, isRoleOperationEnvelope, reduceRoleOperations, resolveRole, } from '../src/services/architectureV2/roles.js';
import { isModerationEnvelope, reduceModerationRecords, } from '../src/services/architectureV2/moderation.js';
...
import { PRIMARY_SYSOP_ADDRESS, forumRolesService, } from '../src/services/qdn/forumRolesService.js';
...
const roleRecord = (input) => {
    ...
    return {
        ...
        envelope: buildRoleOperationEnvelope({
            ...
            prior: input.prior,
        }, input.id, input.clientCreatedAt ?? '9999-01-01T00:00:00.000Z'),
        transaction: {
            type: 'ARBITRARY',
            method: 'PUT',
            signature,
...
const reduce = (records) => reduceRoleOperations({
    primarySysOpAddress: 'ROOT',
    bootstrapRegistry,
    bootstrapMetadata,
    records,
...
assert(resolveRole('LEGACY-ADMIN', bootstrap.registry) === 'Admin' &&
    resolveRole('LEGACY-MODERATOR', bootstrap.registry) === 'Moderator', 'trusted legacy assignments survive bootstrap migration');
...
assert(resolveRole('SUPER', afterSuper.registry) === 'SuperAdmin', 'SysOp grant');
...
assert(resolveRole('ADMIN', afterAdmin.registry) === 'Admin', 'delegated SuperAdmin grant');
...
assert(resolveRole('MODERATOR', afterModerator.registry) === 'Moderator', 'delegated Admin lower-role grant');
...
assert(afterRepeat.registry.moderators.filter((value) => value === 'MODERATOR')
    .length === 1, 'repeated assignment is idempotent');
...
assert(resolveRole('MODERATOR', afterRevoke.registry) === 'Member', 'revocation');
...
assert(reduce([
    ...
    revokeModerator,
    repeatedRevoke,
]).audit.length === 6, 'repeated revocation is deterministic and idempotent');
const rejectionCode = (record) => reduce([record]).diagnostics[0]?.code;
assert(rejectionCode(roleRecord({
    id: 'bad-member',
    created: 20,
    actorName: 'member',
    actorAddress: 'MEMBER',
    ...
    role: 'Moderator',
    prior: bootstrap.checkpoint,
})) === 'ROLE_INSUFFICIENT_PRIOR_ROLE', 'Member cannot assign roles');
assert(rejectionCode(roleRecord({
    id: 'bad-moderator',
    created: 20,
    ...
    role: 'Moderator',
    prior: bootstrap.checkpoint,
})) === 'ROLE_INSUFFICIENT_PRIOR_ROLE', 'Moderator cannot assign roles');
...
assert(rejectionCode(selfEscalation) === 'ROLE_SELF_ESCALATION_ATTEMPT', 'self escalation rejected');
...
assert(rejectionCode(illegalAbove) === 'ROLE_FORBIDDEN_ASSIGNMENT', 'grant above actor rejected');
...
assert(rejectionCode(rootMutation) === 'ROLE_PROTECTED_SYSOP_MUTATION', 'fixed root protected');
...
assert(reduceRoleOperations({
    primarySysOpAddress: 'ROOT',
    bootstrapRegistry: peerBootstrap,
    bootstrapMetadata,
    records: [peerMutation],
}).diagnostics[0]?.code === 'ROLE_TARGET_HIERARCHY_VIOLATION', 'peer/higher target protected');
assert(rejectionCode({
    ...illegalAbove,
    metadata: { ...illegalAbove.metadata, publisherName: 'forger' },
}) === 'ROLE_FORGED_ACTOR', 'forged embedded actor rejected before authorization');
assert(rejectionCode(roleRecord({
    ...{
        id: 'bad-wallet',
        created: 20,
...
})) === 'ROLE_PUBLISHER_WALLET_MISMATCH', 'publisher wallet mismatch rejected');
assert(rejectionCode(roleRecord({
    id: 'bad-publisher',
    created: 20,
    actorName: 'root',
...
})) === 'ROLE_UNTRUSTED_PUBLISHER', 'untrusted QDN publisher rejected');
...
const consumeForgedGrant = roleRecord({
    ...
    prior: {
        ...
        previousOperationSignature: unauthorizedGrant.metadata.latestSignature ?? null,
    },
});
const badChain = reduce([unauthorizedGrant, consumeForgedGrant]);
assert(badChain.audit.length === 0 &&
    badChain.diagnostics.some((item) => item.code === 'ROLE_LINEAGE_MISMATCH'), 'authorization is prior-state sequential and cannot consume a forged grant');
...
assert(JSON.stringify(branchForward.registry) ===
    ...
    resolveRole('BRANCH-B', branchForward.registry) === 'Member', 'trusted metadata tie-breakers are deterministic and client time is ignored');
const conflictingCopy = {
    ...branchA,
    envelope: buildRoleOperationEnvelope({ ...branchA.envelope.body, targetAddress: 'CONFLICT' }, branchA.envelope.recordId),
};
assert(reduce([branchA, conflictingCopy]).diagnostics[0]?.code ===
    'ROLE_OPERATION_CONFLICT', 'conflicting duplicate resource is quarantined');
assert(!isRoleOperationEnvelope({
    ...grantSuper.envelope,
...
const moderationResources = [];
const moderationPayloads = new Map();
...
testGlobal.qdnRequest = async (payload) => {
    ...
    if (action === 'SEARCH_QDN_RESOURCES') {
        ...
        if (identifier === 'qdbm-v2-role-') {
            ...
            return roleResources.slice(Number(payload.offset ?? 0), Number(payload.offset ?? 0) + 100);
        }
    ...
    if (action === 'FETCH_QDN_RESOURCE') {
        ...
        if (identifier === runtimePost.recordId)
            return runtimePost;
        if (rolePayloads.has(identifier))
            return rolePayloads.get(identifier);
    ...
    if (action === 'FETCH_NODE_API') {
        const signature = decodeURIComponent(String(payload.path).split('/').pop() ?? '');
        const transaction = transactions.get(signature);
        if (!transaction)
            throw new Error('mock transaction unavailable');
        return transaction;
    }
...
const runtimeBootstrap = await forumRolesService.loadTrustedRoleAuthorizationState({
    force: true,
});
assert(runtimeBootstrap.status === 'VERIFIED' &&
    resolveRole('LEGACY-RUNTIME-ADMIN', runtimeBootstrap.registry) ===
        'Admin' &&
    runtimeBootstrap.diagnostics.some((item) => item.code === 'ROLE_LEGACY_DELEGATED_SNAPSHOT_IGNORED'), 'canonical legacy registry bootstraps while delegated snapshots remain untrusted evidence');
const publishRole = async (input) => {
    const result = await forumRolesService.publishRoleOperation(input);
    assert(result.ok && !('partial' in result), `role publication persists: ${input.action} ${input.role}`);
...
assert(resolveRole('RUNTIME-SUPER', reloadedGrant.registry) === 'SuperAdmin' &&
    resolveRole('RUNTIME-ADMIN', reloadedGrant.registry) === 'Admin' &&
    resolveRole('RUNTIME-MODERATOR', reloadedGrant.registry) === 'Moderator' &&
    reloadedGrant.audit.length === 3, 'delegated SuperAdmin/Admin grants survive full discovery and reduction reload');
...
assert(moderationPublish.ok, 'delegated role authorizes Phase 4 moderation after reload');
const moderationBeforeRevoke = await forumQdnService.loadV2ModerationState();
assert(moderationBeforeRevoke.targets['role-runtime-post']?.pinned === true, 'published moderation operation reloads under delegated role history');
runtimeState = await publishRole({
    action: 'revoke',
    role: 'Moderator',
    targetAddress: 'RUNTIME-MODERATOR',
...
assert(resolveRole('RUNTIME-MODERATOR', runtimeState.registry) === 'Member', 'delegated revoke persisted');
const reloadedRevoke = await forumRolesService.loadTrustedRoleAuthorizationState({
    force: true,
});
assert(resolveRole('RUNTIME-MODERATOR', reloadedRevoke.registry) === 'Member', 'revocation survives full reload');
...
assert(revokedModerationRejected &&
    moderationResources.length === publicationsBeforeRevokedModeration, 'revoked actor cannot publish a new moderation operation');
const moderationAfterRevoke = await forumQdnService.loadV2ModerationState();
assert(moderationAfterRevoke.targets['role-runtime-post']?.pinned === true, 'later revocation does not retroactively invalidate prior valid moderation');
...
assert(rejectedPublication.ok === false &&
    !rejectedPublication.published &&
    (await forumRolesService.loadTrustedRoleAuthorizationState({ force: true }))
        .audit.length === auditBeforeFailure, 'publication rejection leaves trusted state unchanged');
...
assert(partialPublication.ok &&
    'partial' in partialPublication &&
    partialPublication.partial.pending === 'role-state-refresh', 'published authority plus refresh failure is explicit retryable partial success');
failRoleDiscovery = false;
const recovered = await forumRolesService.loadTrustedRoleAuthorizationState({
    force: true,
});
assert(resolveRole('RECOVERABLE-TARGET', recovered.registry) === 'Moderator', 'partial publication is recoverable by complete reload');
const historicalModeration = moderationResources[0];
const historicalEnvelope = moderationPayloads.get(historicalModeration.identifier);
assert(isModerationEnvelope(historicalEnvelope), 'historical moderation payload remains schema-valid');
assert(reduceModerationRecords([
    {
        metadata: {
    ...
    validateWalletBinding: (name, wallet) => runtimeWallets[name] === wallet
        ? { ok: true }
        : { ok: false, code: 'IDENTITY_UNVERIFIED', detail: 'wallet' },
}, recovered).targets['role-runtime-post']?.pinned === true, 'historical role checkpoint is consumed deterministically by moderation');
assert(bridgeCalls.some((call) => call.action === 'FETCH_NODE_API'), 'immutable Core transaction evidence participates in role verification');
const trustedBootstrapTransaction = transactions.get('runtime-bootstrap-signature');
transactions.set('runtime-bootstrap-signature', {
...
const transferredBootstrap = await forumRolesService.loadTrustedRoleAuthorizationState({ force: true });
assert(transferredBootstrap.status === 'UNAVAILABLE' &&
    transferredBootstrap.diagnostics.some((item) => item.code === 'ROLE_BOOTSTRAP_TRUST_FAILURE'), 'current/cached name ownership cannot replace immutable primary transaction proof');
