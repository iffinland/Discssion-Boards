import { QortiumRequestError, requestQortium, resolveQortiumRequestBridge, } from '../src/services/qortium/qortiumClient.js';
import { QDN_HOME_SOURCE_MAX_BYTES, QDN_INLINE_FILE_MAX_BYTES, QdnFilePublicationError, publishQdnFileResource, sanitizeQdnSourceFilename, } from '../src/services/qortium/qdnFilePublication.js';
import { encodeQdnImageTag, parseQdnImageTagPayload, } from '../src/services/forum/richText.js';
import { createAttachmentSignature, getAttachmentSizeLimit, isAllowedAttachmentFile, } from '../src/services/forum/attachments.js';
import { encodeQdnVideoTag, parseQdnVideoTagPayload, } from '../src/services/forum/videoEmbed.js';
import { forumQdnService } from '../src/services/qdn/forumQdnService.js';
import { buildV2OwnerEditEnvelope, buildV2PostEnvelope, isV2EntityEnvelope, reduceV2RuntimeRecords, } from '../src/services/architectureV2/runtime.js';
const assert = (condition, detail) => {
    if (!condition)
        throw new Error(`Assertion failed: ${detail}`);
};
const assertEqual = (actual, expected, detail) => {
    assert(Object.is(actual, expected), `${detail}; expected ${String(expected)}, received ${String(actual)}`);
};
...
test('bridge reports unavailable when no scope exposes qdnRequest', () => {
    assertEqual(resolveQortiumRequestBridge({ globalScope: {}, windowScope: {} }).status, 'UNAVAILABLE', 'empty scopes must be unavailable');
});
...
test('non-callable bridge is reported as malformed', () => {
    assertEqual(resolveQortiumRequestBridge({
        globalScope: { qdnRequest: { request: requestFunction } },
    }).status, 'MALFORMED', 'object-shaped bridge is not the Home contract');
});
test('throwing bridge getter is reported as inaccessible', () => {
    ...
    assertEqual(resolveQortiumRequestBridge({ globalScope: scope }).status, 'INACCESSIBLE', 'throwing bridge getter must be inaccessible');
});
...
test('cross-origin top access failure is handled without throwing', () => {
    ...
    assertEqual(resolveQortiumRequestBridge({ globalScope: {}, windowScope }).status, 'INACCESSIBLE', 'top access failure must be controlled');
});
test('requestQortium invokes the documented callable bridge shape', async () => {
    ...
    try {
        ...
        if (existing)
            Object.defineProperty(globalThis, 'qdnRequest', existing);
        else
...
test('publishing outside Home fails with a stable bridge diagnostic', async () => {
    ...
    Reflect.deleteProperty(globalThis, 'qdnRequest');
    try {
        const first = await assertRejectsCode(() => requestQortium({ action: 'PUBLISH_QDN_RESOURCE' }), 'BRIDGE_UNAVAILABLE');
        assert(first.message.includes('Open this app through Qortium Home'), 'outside-Home error must tell the user how to publish');
    }
    finally {
        if (existing)
            Object.defineProperty(globalThis, 'qdnRequest', existing);
...
const fakeFile = (size, name = 'sample.zip', type = 'application/zip') => ({
    name,
    type,
    size,
    lastModified: 123456,
...
const createHarness = (options = {}) => {
    ...
    const request = async (payload) => {
        ...
        if (payload.action === 'PUBLISH_QDN_RESOURCE') {
            ...
            published = true;
            return (options.publishResponse ?? {
                accepted: true,
                transactionSignature: 'signature-1',
            });
        }
        if (payload.action === 'SEARCH_QDN_RESOURCES') {
            if (options.confirmation === 'unavailable')
                return { unavailable: true };
            if (options.confirmation === 'missing')
                return [];
...
test('small file publishes through bounded inline base64', async () => {
    ...
    const publish = harness.calls.find((call) => call.action === 'PUBLISH_QDN_RESOURCE');
...
test('100 MiB boundary is accepted by the token transport', async () => {
    ...
    assertEqual((await harness.run()).transport, 'home-source-token', 'verified Home boundary must use token transport');
});
test('resource over the verified Home cap is rejected before bridge use', async () => {
    ...
    assertEqual(harness.calls.length, 0, 'oversized resource must not use bridge');
});
test('browser publication refuses to start without durable recovery storage', async () => {
    ...
    try {
        await assertRejectsCode(() => publishQdnFileResource({
...
test('source-token action failure never falls back to base64', async () => {
    ...
    assert(!calls.some((call) => 'data64' in call), 'source-token failure must not create inline payload');
});
...
test('Home-compatible filename normalization is deterministic', () => {
    assertEqual(sanitizeQdnSourceFilename(' bad:*  name?.zip '), 'bad__ name_.zip', 'filename must match Home normalization');
});
...
test('malformed publication response is a recoverable publication failure', async () => {
    ...
    assert(error instanceof QdnFilePublicationError && Boolean(error.recovery), 'malformed result must preserve the identifier for safe retry');
});
test('explicit Home rejection is not treated as an ambiguous submit', async () => {
    ...
    await assertRejectsCode(harness.run, 'PUBLICATION_REJECTED');
    assertEqual(harness.store.records.size, 0, 'explicit rejection clears recovery');
});
test('ambiguous publication failure preserves identifier recovery', async () => {
    ...
    const error = await assertRejectsCode(harness.run, 'POSSIBLE_ALREADY_PUBLISHED');
    assert(error instanceof QdnFilePublicationError && Boolean(error.recovery), 'ambiguous failure must expose recovery');
...
test('accepted but unconfirmed publication returns recoverable state', async () => {
    ...
    const error = await assertRejectsCode(harness.run, 'CONFIRMATION_UNAVAILABLE');
    assert(error instanceof QdnFilePublicationError &&
        error.recovery?.stage === 'CONFIRMATION_UNAVAILABLE', 'accepted publication must preserve confirmation recovery');
});
test('retry confirms the stored identifier without republishing', async () => {
    ...
    assertEqual(second.resource.identifier, first.resource.identifier, 'stored identifier must remain stable');
    assert(!secondCalls.some((call) => call.action === 'PUBLISH_QDN_RESOURCE'), 'confirmed retry must not republish');
});
test('ambiguous recovery with a missing exact resource never republishes automatically', async () => {
    ...
    const error = await assertRejectsCode(retry.run, 'POSSIBLE_ALREADY_PUBLISHED');
    assert(error instanceof QdnFilePublicationError, 'recovery must be exposed');
    assertEqual(error.recovery?.resource.identifier, failed.resource.identifier, 'recovery must retain the same QDN identifier');
    assert(!retry.calls.some((call) => call.action === 'PUBLISH_QDN_RESOURCE'), 'ambiguous recovery must not republish automatically');
});
test('unavailable preflight fails before first publication', async () => {
    ...
    assert(!harness.calls.some((call) => call.action === 'PUBLISH_QDN_RESOURCE'), 'unavailable preflight must fail closed');
});
test('bridge errors remain distinct from QDN confirmation errors', async () => {
    const harness = createHarness();
    const bridgeError = new QortiumRequestError('BRIDGE_UNAVAILABLE', 'Open through Home.');
    await assertRejectsCode(() => publishQdnFileResource({
        file: harness.file,
        service: harness.resource.service,
        name: harness.resource.name,
        identifier: harness.resource.identifier,
    }, { request: async () => Promise.reject(bridgeError) }), 'BRIDGE_UNAVAILABLE');
});
...
test('legacy attachment references remain readable without migration', () => {
    ...
    assertEqual(createAttachmentSignature(legacyAttachment), 'qdbm-att-existing:archive.zip', 'legacy attachment signature must remain unchanged');
});
test('legacy video tag format remains parseable', () => {
    ...
    assertEqual(parsed?.identifier, 'qdbm-video-existing', 'video id must survive');
});
test('attachment type and product-size rules remain explicit', () => {
    ...
    assert(!isAllowedAttachmentFile(unsupported), 'PNG attachment is unsupported');
    assertEqual(getAttachmentSizeLimit(zip), 10 * 1024 * 1024, 'ZIP product limit must remain 10 MiB');
});
...
test('confirmed attachment references enter Post state only through V2 owner policy', () => {
    ...
    const identity = {
        ...
        validateWalletBinding: (_name, wallet) => wallet === 'wallet-1'
            ? { ok: true }
            : {
                ok: false,
                code: 'IDENTITY_UNVERIFIED',
    ...
    assert(authoritative?.entityType === 'post' &&
        authoritative.attachments?.[0]?.identifier === attachment.identifier, 'validated owner edit must establish the attachment reference');
    ...
    assert(rejected.diagnostics.some((item) => item.code === 'FORBIDDEN_FIELD'), 'malformed attachment owner edit must be rejected by field policy');
});
test('forum image, attachment, and video services use the centralized transport', async () => {
    ...
    try {
        ...
        const imageReference = await forumQdnService.publishPostImage(image, 'Alice');
        const attachmentReference = await forumQdnService.publishPostAttachment(largeFiles[0], 'Alice');
        const videoReference = await forumQdnService.publishPostVideo(largeFiles[1], 'Alice');
        assert(imageReference.identifier.includes('-img-'), 'image service must return its media identifier');
        assert(attachmentReference.identifier.includes('-att-'), 'attachment service must return its media identifier');
        assert(videoReference.identifier.includes('-video-'), 'video service must return its media identifier');
        const mediaPublishes = bridgeCalls.filter((call) => call.action === 'PUBLISH_QDN_RESOURCE');
        ...
        assert(mediaPublishes.slice(1).every((call) => 'sourceToken' in call), 'large attachment and video use source tokens');
        assert(mediaPublishes.slice(1).every((call) => !('data64' in call)), 'large service paths never include inline data');
    }
    finally {
        if (existing)
            Object.defineProperty(globalThis, 'qdnRequest', existing);
...
test('bridge diagnostics are stable across repeated resolution', () => {
    ...
    assertEqual(resolveQortiumRequestBridge(environment).status, resolveQortiumRequestBridge(environment).status, 'bridge classification must not change across retry/reload');
});
let passed = 0;
