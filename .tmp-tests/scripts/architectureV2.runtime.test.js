...
const identity = {
    ...
    validateWalletBinding: (_name, wallet) => wallet
        ? { ok: true }
        : {
            ok: false,
            code: 'IDENTITY_UNVERIFIED',
...
const metadata = (id, publisher = 'alice', created = 1) => ({
    service: 'DOCUMENT',
...
assert(JSON.stringify(forward.authoritative) ===
    JSON.stringify(reverse.authoritative), 'reduction is order independent');
...
assert(edited.authoritative.entities['topic-1']?.entityType === 'topic' &&
    edited.authoritative.entities['topic-1'].title === 'Changed', 'owner edit survives reduction');
const unrelated = applyOwnerEdit(forward.authoritative, metadata('edit', 'mallory', 4), {
    operation: 'owner-edit',
    targetType: 'post',
    targetId: 'post-1',
...
assert(unrelated.quarantined[unrelated.quarantined.length - 1]?.code ===
    'IDENTITY_UNVERIFIED', 'unrelated publisher rejected');
const forbidden = applyOwnerEdit(forward.authoritative, metadata('edit', 'alice', 4), {
    operation: 'owner-edit',
    targetType: 'post',
    targetId: 'post-1',
    publisherName: 'alice',
...
assert(forbidden.quarantined[forbidden.quarantined.length - 1]?.code ===
    'FORBIDDEN_FIELD', 'forbidden fields rejected');
