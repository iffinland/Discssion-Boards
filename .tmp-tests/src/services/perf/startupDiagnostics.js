...
const safeText = (value, maxLength = 180) => typeof value === 'string' && value.trim()
    ? value
        .trim()
        .replace(/(private.?key|seed|secret|password|auth(?:entication)?(?:token)?|signature)\s*[:=]\s*[^\s,;]+/gi, '$1=[redacted]')
        .slice(0, maxLength)
    : undefined;
export const isStartupDiagnosticsEnabled = () => enabled;
export const configureStartupDiagnosticsForTest = (nextEnabled) => {
    enabled = nextEnabled;
};
...
export const recordStartupEvent = (name, input = {}) => {
    if (!enabled)
        return;
    const event = {
        sequence: ++sequence,
...
export const setStartupState = (state, completion) => {
    currentState = state;
...
export const beginStartupSpan = (name, input = {}) => {
    const spanStartedAt = monotonicNow();
    recordStartupEvent(name, input);
    return (endName, result = {}) => {
...
export const getStartupDiagnosticsReport = () => {
...
export const subscribeStartupDiagnostics = (listener) => {
