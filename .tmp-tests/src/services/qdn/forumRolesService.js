...
import { buildRoleOperationEnvelope, isRoleOperationEnvelope, reduceRoleOperations, validateRoleMutationAgainstPriorState, } from '../architectureV2/roles.js';
...
let trustedRoleStateCache = { value: null, updatedAt: 0, inflight: null };
...
const normalizeAddressList = (input) => {
...
export const createDefaultRoleRegistry = () => ({
...
const parseRoleRegistryPayload = (raw) => {
    if (!isObject(raw) ||
    ...
    return {
        ...
        registry: {
            primarySysOpAddress: typeof raw.registry.primarySysOpAddress === 'string' &&
...
const toForumRoleRegistry = (payload) => {
    ...
    return {
        ...
        admins: payload.registry.admins.filter((address) => !payload.registry.moderators.includes(address) &&
            !sysOps.includes(address)),
        moderators: payload.registry.moderators.filter((address) => !payload.registry.admins.includes(address) && !sysOps.includes(address)),
...
const searchCanonicalRoleResources = async () => {
    ...
    return {
        resources: discovery.items.filter((resource) => resource.identifier === PRIMARY_ROLE_IDENTIFIER),
...
const toTrustedMetadata = (resource) => {
...
const compareTrustedRoleResources = (left, right) => {
    ...
    const signature = (right.metadata.latestSignature ?? '').localeCompare(left.metadata.latestSignature ?? '');
    if (signature)
        return signature;
    const name = right.metadata.publisherName.localeCompare(left.metadata.publisherName);
    return (name || right.metadata.identifier.localeCompare(left.metadata.identifier));
};
...
const parseCoreRoleTransaction = (raw) => {
    if (!isObject(raw))
        return null;
    const transaction = raw;
    if (transaction.type !== 'ARBITRARY' ||
    ...
    return {
        ...
        blockHeight: typeof transaction.blockHeight === 'number'
            ? transaction.blockHeight
            : null,
        blockSequence: typeof transaction.blockSequence === 'number'
            ? transaction.blockSequence
            : null,
    };
};
const fetchRoleTransaction = async (signature) => parseCoreRoleTransaction(await requestQortium({
...
const assertIdentifierLength = (identifier) => {
    if (identifier.length > MAX_SAFE_QDN_IDENTIFIER_LENGTH)
        throw new Error(`Role operation identifier exceeds ${MAX_SAFE_QDN_IDENTIFIER_LENGTH} characters.`);
};
const verifyPublication = async (ownerName, identifier) => {
...
const verifyRoleOperationPublication = async (ownerName, identifier) => {
    for (let attempt = 1; attempt <= VERIFY_RETRIES; attempt += 1) {
        try {
            const raw = await requestQortium({
                action: 'FETCH_QDN_RESOURCE',
            ...
            if (isRoleOperationEnvelope(parseJsonLike(raw)))
                return;
        }
        catch {
    ...
    throw new Error('Role operation was submitted but its QDN payload is not readable yet.');
};
...
export const resolveRoleForAddress = (address, registry) => {
    if (!address?.trim()) {
        return 'Member';
    }
...
const loadTrustedBootstrap = async () => {
    let trustedNames;
    try {
    ...
    const trustedResources = resources.filter((resource) => trustedNameSet.has(resource.name.trim().toLowerCase()));
    const delegatedDiagnostics = resources
        .filter((resource) => !trustedNameSet.has(resource.name.trim().toLowerCase()))
        .map((resource) => ({
    ...
    if (trustedResources.length === 0)
        return {
            ...
            detail: 'no primary-owned legacy registry exists; the fixed primary SysOp is the bootstrap state',
        };
        ...
        return {
            ...
            diagnostics: [
                ...delegatedDiagnostics,
                {
                    code: 'ROLE_BOOTSTRAP_TRUST_FAILURE',
                    identifier: PRIMARY_ROLE_IDENTIFIER,
                    detail: 'a primary-owned canonical registry lacks trusted Core metadata',
                },
            ],
            detail: 'canonical role bootstrap metadata is incomplete',
        };
    const selected = ordered[0];
    let bootstrapTransaction;
    try {
        bootstrapTransaction = await fetchRoleTransaction(selected.metadata.latestSignature ?? '');
    }
    catch {
        bootstrapTransaction = null;
    }
    const bootstrapTimestamp = selected.metadata.updated ?? selected.metadata.created;
        ...
        return {
            ...
            diagnostics: [
                ...delegatedDiagnostics,
                {
                    ...
                    detail: 'latest canonical registry revision is not transaction-bound to the fixed primary SysOp',
                },
            ],
            detail: 'canonical role bootstrap lacks immutable primary-SysOp transaction proof',
        };
    try {
        const payload = parseRoleRegistryPayload(await fetchResource(selected.resource.name, selected.resource.identifier));
        if (!payload)
            return {
                status: 'UNAVAILABLE',
                registry: createDefaultRoleRegistry(),
    ...
    for (const resource of discovery.resources) {
        ...
        if (!transaction) {
            diagnostics.push({
                ...
                identifier: resource.identifier,
                detail: 'immutable role-operation transaction identity evidence is unavailable',
            });
            unavailable = true;
            continue;
        }
...
export const forumRolesService = {
    ...
    async publishRoleRegistry(registry, ownerName) {
        ...
        if (resolvedWallet?.trim() !== PRIMARY_SYSOP_ADDRESS)
            throw new Error('[ROLE_UNTRUSTED_PUBLISHER] only a QDN name currently owned by the fixed primary SysOp may publish the legacy bootstrap registry');
        const updatedAt = Date.now();
        const sanitizedRegistry = {
            primarySysOpAddress: PRIMARY_SYSOP_ADDRESS,
            sysOps: normalizeAddressList(registry.sysOps),
            admins: normalizeAddressList(registry.admins).filter((address) => !normalizeAddressList(registry.sysOps).includes(address)),
            moderators: normalizeAddressList(registry.moderators).filter((address) => !normalizeAddressList(registry.admins).includes(address) &&
    ...
    async publishRoleOperation(input) {
        ...
        const authorized = validateRoleMutationAgainstPriorState(operation, current.registry, current.checkpoint);
        ...
        const recordId = `${ROLE_OPERATION_PREFIX}${generateForumEntityId('role', actorName)}`;
        ...
        return {
            ...
            envelope,
            partial: { pending: 'role-state-refresh', retryable: true },
            detail: 'role operation is published but has not appeared in complete trusted discovery yet',
        };
    },
};
