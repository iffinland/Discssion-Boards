import { buildTipReferenceEnvelope, buildTipReferenceIdentifier, buildTipReferencePrefix, isTipReferenceEnvelope, loadTipReferences, normalizeQortAmount, resolveTipDisplay, } from '../architectureV2/tips.js';
...
import { getUserAccount, resolveNameWalletAddressUncached, } from '../qortium/walletService.js';
...
export const isTipRecovery = (value) => {
    if (!isObject(value) || !isObject(value.body))
        return false;
    if (!Object.keys(value).every((key) => ['schema', 'schemaVersion', 'phase', 'recordId', 'body'].includes(key)) ||
...
export const finalizeTipDerivedState = async (result, refreshDerived) => {
...
const lookupFailure = (error) => {
    ...
    return /TRANSACTION_UNKNOWN|transaction unknown|(?:^|\D)311(?:\D|$)/i.test(detail)
        ? { status: 'not-found', detail }
        : { status: 'unavailable', detail };
};
export const parseCoreQortTransfer = (raw, expectedSignature) => {
    if (!isObject(raw))
        return {
            status: 'invalid',
    ...
    if (typeof raw.blockHeight !== 'number' ||
    ...
    if (raw.approvalStatus !== 'NOT_REQUIRED' &&
    ...
    return {
        ...
        evidence: {
            ...
            blockSequence: typeof raw.blockSequence === 'number' ? raw.blockSequence : null,
            approvalStatus: raw.approvalStatus,
        },
    };
};
export const parseCoreReferenceTransaction = (raw, expectedSignature) => {
    if (!isObject(raw))
        return {
            status: 'invalid',
    ...
    if (raw.type !== 'ARBITRARY' ||
        ...
        return {
            status: 'invalid',
            code: 'TIP_REFERENCE_TRANSACTION_MISMATCH',
            detail: 'QDN reference publication transaction is unconfirmed or mismatched',
        };
    if (raw.approvalStatus === 'REJECTED' ||
        raw.approvalStatus === 'EXPIRED' ||
        raw.approvalStatus === 'INVALID')
    ...
    if (raw.approvalStatus !== 'NOT_REQUIRED' &&
        ...
        return {
            status: 'invalid',
            code: 'TIP_REFERENCE_TRANSACTION_MISMATCH',
            detail: 'QDN tip-reference publication has an unsupported approval status',
        };
    return {
        status: 'found',
        evidence: {
...
const parseSendResponse = (raw) => {
    ...
    const response = raw;
    const signature = typeof response.transactionSignature === 'string' &&
        response.transactionSignature.trim()
        ? response.transactionSignature.trim()
        : null;
    return {
        signature,
        valid: response.accepted === true &&
...
const assertIdentifierLength = (identifier) => {
    if (identifier.length > MAX_SAFE_QDN_IDENTIFIER_LENGTH)
        throw new Error(`Tip reference identifier exceeds ${MAX_SAFE_QDN_IDENTIFIER_LENGTH} characters.`);
};
const targetRecipient = (authority, postId) => {
...
const recoveryFor = (phase, recordId, body) => ({
    schema: 'qdb-tip-recovery',
    schemaVersion: 1,
    phase,
    recordId,
...
export const createForumTipsService = (dependencies) => {
    const load = async (authority) => {
        const discovery = await dependencies.discoverResources();
        return loadTipReferences(discovery.resources, authority.authoritative, {
            fetchPayload: dependencies.fetchPayload,
            expectedIdentifier: (signature) => buildTipReferenceIdentifier(FORUM_NAMESPACE, signature),
    ...
    const verifyCurrentSender = async (body) => {
        ...
        return (account.address?.trim() === body.senderAddress.trim() &&
            resolved?.trim() === body.senderAddress.trim());
    };
    const continueAfterPayment = async (body, recordId, authority) => {
        const payment = await dependencies.fetchPaymentTransaction(body.transactionSignature);
        ...
        if (payment.evidence.recipient !== body.recipientAddress)
            return {
                ...
                detail: 'confirmed payment recipient differs from the authoritative Post owner',
            };
        if (payment.evidence.amountQort !== body.amountQort)
            return {
                ...
                detail: 'confirmed payment amount differs from the requested tip amount',
            };
        let existing;
        try {
        ...
        try {
            alreadyPublished = await dependencies.referenceExists(body.senderName, recordId);
        }
        catch (error) {
            return {
                ok: true,
        ...
        if (alreadyPublished) {
            const rejection = existing.diagnostics.find((entry) => entry.identifier === recordId);
            ...
            return {
                ...
                retryable: true,
                recovery: recoveryFor('reference-refresh', recordId, body),
                detail: 'tip reference already exists and is awaiting complete verification; it was not republished',
            };
        }
        if (!(await verifyCurrentSender(body)))
            return {
                ...
                detail: 'current sender name, selected wallet, and payment sender cannot be bound',
            };
        try {
        ...
        const rejection = reloaded.diagnostics.find((entry) => entry.identifier === recordId);
        ...
        return {
            ...
            retryable: true,
            recovery: recoveryFor('reference-refresh', recordId, body),
            detail: 'tip payment is confirmed but its reference has not appeared in complete discovery',
        };
    };
    return {
        resolveRecipient(authority, postId) {
            return targetRecipient(authority, postId);
        },
        load,
        async apply(posts, authority) {
        ...
        async submit(input) {
            ...
            if (!amountQort)
                return {
                    ...
                    code: 'TIP_AMOUNT_MISMATCH',
                    detail: 'tip amount must be positive with no more than eight decimals',
                };
            ...
            if (!recipient)
                return {
                    ...
                    detail: 'legacy or unavailable Post authority cannot select a tip recipient',
                };
            ...
            if (!(await verifyCurrentSender(provisionalBody)))
                return {
                    ...
                    status: 'PAYMENT_FAILED',
                    code: 'TIP_WALLET_NAME_UNAVAILABLE',
                    detail: 'selected account and current sender QDN name/wallet binding could not be verified',
                };
            let rawResponse;
            try {
                rawResponse = await dependencies.sendQort(recipient.address, amountQort);
            }
            catch (error) {
                return {
                    ok: false,
                    ...
                    status: 'PAYMENT_FAILED',
                    code: 'TIP_TRANSACTION_INVALID',
                    detail: error instanceof Error ? error.message : 'QORT transfer failed',
                };
            }
            ...
            const recordId = await buildTipReferenceIdentifier(FORUM_NAMESPACE, response.signature);
            assertIdentifierLength(recordId);
            return continueAfterPayment(body, recordId, input.authority);
        },
        async retry(recovery, authority) {
            ...
            if (!recipient ||
                ...
                return {
                    ...
                    code: 'TIP_RECIPIENT_MISMATCH',
                    detail: 'authoritative Post owner changed or is unavailable; retry failed closed',
                };
            const expectedRecordId = await buildTipReferenceIdentifier(FORUM_NAMESPACE, recovery.body.transactionSignature);
...
const discoverTipResources = async () => {
...
const fetchPayload = async (resource) => {
    ...
    return parseJsonLike(await fetchWithQdnReadyFallback(FORUM_SERVICE, resource.name, resource.identifier, fetcher));
};
const fetchCoreTransaction = async (signature) => requestQortium({
...
export const forumTipsService = createForumTipsService({
    ...
    referenceExists: async (publisherName, identifier) => {
        ...
        if (result.completeness !== 'complete')
            throw new Error('[PARTIAL_DISCOVERY] exact tip-reference lookup is incomplete');
        return result.items.some((resource) => resource.name?.trim().toLowerCase() ===
    ...
    fetchReferenceTransaction: async (signature) => {
        try {
            return parseCoreReferenceTransaction(await fetchCoreTransaction(signature), signature);
        }
        catch (error) {
    ...
    fetchPaymentTransaction: async (signature) => {
        try {
            return parseCoreQortTransfer(await fetchCoreTransaction(signature), signature);
        }
        catch (error) {
            return lookupFailure(error);
        }
    ...
    sendQort: (recipient, amountQort) => requestQortium({
        action: 'SEND_COIN',
        coin: 'QORT',
        recipient,
        amount: amountQort,
