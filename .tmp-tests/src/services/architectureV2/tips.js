...
export const QORT_ATOMIC_FACTOR = 100000000n;
const isRecord = (value) => typeof value === 'object' && value !== null && !Array.isArray(value);
const hasOnlyKeys = (value, allowed) => Object.keys(value).every((key) => allowed.includes(key));
const normalizeSignature = (value) => value.trim();
export const qortToAtomic = (value) => {
    let raw;
...
export const classifyInvalidTipReference = (value) => {
    if (!isRecord(value) || !isRecord(value.body))
        return 'TIP_MALFORMED_REFERENCE';
    if (typeof value.body.transactionSignature !== 'string' ||
...
export const buildTipReferenceEnvelope = (body, recordId, clientCreatedAt = new Date().toISOString()) => ({
    schema: 'qdb-v2',
    schemaVersion: 2,
    kind: 'operation',
...
export const buildTipReferencePrefix = (namespace) => `${namespace}-v2-tip-`;
export const buildTipReferenceIdentifier = async (namespace, transactionSignature) => `${buildTipReferencePrefix(namespace)}${(await sha256Hex(normalizeSignature(transactionSignature))).slice(0, 40)}`;
const sameAddress = (left, right) => left.trim() === right.trim();
const sameName = (left, right) => normalizeName(left) === normalizeName(right);
const orderTipRecords = (left, right) => left.paymentTransaction.blockHeight - right.paymentTransaction.blockHeight ||
    ...
    left.paymentTransaction.signature.localeCompare(right.paymentTransaction.signature) ||
    left.metadata.identifier.localeCompare(right.metadata.identifier);
const sameAcceptedTip = (left, right) => left.targetId === right.targetId &&
...
const diagnostic = (code, identifier, detail) => ({ code, identifier, detail });
const validateRecord = (record, authority) => {
    const { metadata, envelope, referenceTransaction, paymentTransaction } = record;
    const body = envelope.body;
    const metadataValidation = validateMetadata(metadata);
    if (metadataValidation.ok === false)
        return diagnostic('TIP_MISSING_TRUSTED_METADATA', metadata.identifier, metadataValidation.detail);
    if (metadata.updated !== null)
        return diagnostic('TIP_REFERENCE_REPUBLISHED', metadata.identifier, 'tip references are immutable and may not be updated');
    if (!metadata.latestSignature ||
        ...
        return diagnostic('TIP_REFERENCE_TRANSACTION_MISMATCH', metadata.identifier, 'QDN reference metadata does not match its confirmed publication transaction');
    if (!sameName(metadata.publisherName, body.senderName))
        return diagnostic('TIP_UNAUTHORIZED_PUBLISHER', metadata.identifier, 'only the payment sender may publish its tip reference');
    if (!sameAddress(referenceTransaction.creatorAddress, body.senderAddress))
        return diagnostic('TIP_SENDER_MISMATCH', metadata.identifier, 'tip-reference publisher wallet does not match the claimed sender wallet');
    if (envelope.targetId !== body.targetId ||
        envelope.recordId !== metadata.identifier)
        return diagnostic('TIP_TARGET_MISMATCH', metadata.identifier, 'tip-reference envelope target or record identifier is inconsistent');
    const target = authority.entities[body.targetId];
    if (!target || target.entityType !== 'post')
        return diagnostic('TIP_TARGET_UNAVAILABLE', metadata.identifier, 'tip target is not an authoritative V2 Post');
    if (!sameName(body.recipientName, target.publisherName) ||
        !sameAddress(body.recipientAddress, target.walletAddress))
        return diagnostic('TIP_RECIPIENT_MISMATCH', metadata.identifier, 'tip reference recipient does not match the authoritative Post owner');
    if (paymentTransaction.signature !== body.transactionSignature ||
        ...
        return diagnostic('TIP_TRANSACTION_INVALID', metadata.identifier, 'referenced QORT transfer is not confirmed and valid');
    if (!sameAddress(paymentTransaction.creatorAddress, body.senderAddress))
        return diagnostic('TIP_SENDER_MISMATCH', metadata.identifier, 'payment transaction sender does not match the tip reference');
    if (!sameAddress(paymentTransaction.recipient, body.recipientAddress))
        return diagnostic('TIP_RECIPIENT_MISMATCH', metadata.identifier, 'payment transaction recipient does not match the authoritative Post owner');
    if (paymentTransaction.amountQort !== body.amountQort)
        return diagnostic('TIP_AMOUNT_MISMATCH', metadata.identifier, 'payment transaction amount does not match the tip reference');
    return {
        targetId: body.targetId,
...
const isDiagnostic = (value) => 'code' in value;
const sortDiagnostics = (values) => values.sort((left, right) => left.identifier.localeCompare(right.identifier) ||
    left.code.localeCompare(right.code) ||
    left.detail.localeCompare(right.detail));
export const reduceTipReferences = (records, authority, initialDiagnostics = [], status = 'VERIFIED') => {
    ...
    for (const record of [...records].sort(orderTipRecords)) {
        ...
        if (conflictedSignatures.has(signature))
            continue;
        const existing = acceptedBySignature.get(signature);
        if (existing) {
            diagnostics.push(diagnostic(sameAcceptedTip(existing, checked)
                ? 'TIP_DUPLICATE_REFERENCE'
                : 'TIP_REFERENCE_CONFLICT', checked.referenceIdentifier, sameAcceptedTip(existing, checked)
    ...
    const ordered = [...acceptedBySignature.values()].sort((left, right) => left.trustedPaymentBlockHeight !== right.trustedPaymentBlockHeight
    ...
    for (const tip of ordered) {
        ...
        byTarget[tip.targetId] = {
            ...
            verifiedTotalQort: atomicToQort(nextTips.reduce((total, entry) => total + (qortToAtomic(entry.amountQort) ?? 0n), 0n)),
            tips: nextTips,
        };
    }
    return {
...
export const loadTipReferences = async (resources, authority, dependencies, discoveryComplete = true) => {
    const records = [];
    const diagnostics = [];
    let unavailable = !discoveryComplete;
    if (!discoveryComplete)
        diagnostics.push(diagnostic('TIP_DISCOVERY_INCOMPLETE', '<tip-discovery>', 'tip-reference discovery exceeded its safety budget'));
    for (const resource of resources) {
        ...
        if (typeof resource.name !== 'string' ||
            ...
            diagnostics.push(diagnostic('TIP_MISSING_TRUSTED_METADATA', identifier, 'tip-reference resource lacks trusted Core metadata'));
            continue;
        }
        let payload;
        try {
            ...
            diagnostics.push(diagnostic('TIP_REFERENCE_UNAVAILABLE', identifier, 'tip-reference payload is unavailable'));
            unavailable = true;
            continue;
        }
        if (!isTipReferenceEnvelope(payload)) {
            diagnostics.push(diagnostic(classifyInvalidTipReference(payload), identifier, 'invalid tip-reference envelope'));
            continue;
        }
        const expectedIdentifier = await dependencies.expectedIdentifier(payload.body.transactionSignature);
        if (expectedIdentifier !== identifier || payload.recordId !== identifier) {
            diagnostics.push(diagnostic('TIP_IDENTIFIER_MISMATCH', identifier, 'tip-reference identifier is not the canonical payment-signature identifier'));
            continue;
        }
        ...
        if (referenceLookup.status !== 'found') {
            diagnostics.push(diagnostic(referenceLookup.status === 'invalid'
                ? referenceLookup.code
                : referenceLookup.status === 'not-found'
                    ? 'TIP_REFERENCE_TRANSACTION_MISMATCH'
                    : 'TIP_VERIFICATION_UNAVAILABLE', identifier, referenceLookup.detail));
            if (referenceLookup.status === 'unavailable')
                unavailable = true;
            continue;
        }
        if (paymentLookup.status !== 'found') {
            diagnostics.push(diagnostic(paymentLookup.status === 'invalid'
                ? paymentLookup.code
                : paymentLookup.status === 'not-found'
                    ? 'TIP_TRANSACTION_NOT_FOUND'
                    : 'TIP_VERIFICATION_UNAVAILABLE', identifier, paymentLookup.detail));
            if (paymentLookup.status === 'unavailable' ||
                paymentLookup.status === 'not-found')
                unavailable = true;
            continue;
        }
    ...
    return reduceTipReferences(records, authority, diagnostics, unavailable ? 'UNAVAILABLE' : 'VERIFIED');
};
export const resolveTipDisplay = (targetId, legacyCount, state) => {
    ...
    return {
        ...
        legacyCount: Number.isSafeInteger(legacyCount) && legacyCount > 0 ? legacyCount : 0,
        legacyIsUnverified: true,
        diagnostics: [
            ...state.diagnostics
                .filter((entry) => entry.identifier === targetId ||
                verified?.tips.some((tip) => tip.referenceIdentifier === entry.identifier))
                .map((entry) => ({ code: entry.code, detail: entry.detail })),
            ...(Number.isSafeInteger(legacyCount) && legacyCount > 0
                ? [
                    {
                        code: 'TIP_LEGACY_UNVERIFIED',
                        detail: 'legacy mutable tip counter is historical display data only',
                    },
                ]
                : []),
        ],
