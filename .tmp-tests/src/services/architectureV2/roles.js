...
const isDelegableRole = (value) => value === 'Moderator' || value === 'Admin' || value === 'SuperAdmin';
const isNullableString = (value) => value === null || typeof value === 'string';
export const isRoleOperationEnvelope = (value) => {
    if (!isRecord(value) || !isRecord(value.body))
        return false;
    const body = value.body;
    if (!isRecord(body.prior))
        return false;
...
export const buildRoleOperationEnvelope = (body, recordId, clientCreatedAt = new Date().toISOString()) => ({
    schema: 'qdb-v2',
    schemaVersion: 2,
    kind: 'operation',
...
const normalizeAddresses = (values, excluded) => {
    const seen = new Set();
    return values
        .map((value) => value.trim())
        .filter((value) => {
        if (!value || seen.has(value) || excluded.has(value))
            return false;
        seen.add(value);
        return true;
    });
};
export const normalizeRoleRegistry = (registry) => {
...
export const resolveRole = (address, registry) => {
...
export const roleLineageEquals = (left, right) => left.bootstrapIdentifier === right.bootstrapIdentifier &&
...
const bootstrapCheckpoint = (metadata) => ({
...
export const compareRoleOrder = (left, right) => {
    if (left.created !== right.created)
        return left.created - right.created;
    const signature = (left.latestSignature ?? '').localeCompare(right.latestSignature ?? '');
    if (signature)
        return signature;
...
const effectiveBootstrapMetadata = (metadata) => metadata
    ? {
        ...metadata,
...
const replaceRole = (registry, address, role) => ({
    ...registry,
...
export const validateRoleMutationAgainstPriorState = (operation, registry, checkpoint) => {
...
const validateRecord = (record, registry, checkpoint) => {
    const metadata = validateMetadata(record.metadata);
    if (metadata.ok === false)
        return {
            ok: false,
            diagnostic: diagnostic('ROLE_MISSING_TRUSTED_METADATA', record.metadata.identifier, metadata.detail),
        };
    if (!record.metadata.latestSignature?.trim())
        return {
            ok: false,
            diagnostic: diagnostic('ROLE_MISSING_TRUSTED_METADATA', record.metadata.identifier, 'role operation requires a trusted latest transaction signature'),
        };
    if (record.metadata.updated !== null)
        return {
            ok: false,
            diagnostic: diagnostic('ROLE_RESOURCE_REPUBLISHED', record.metadata.identifier, 'role operations are append-only and cannot reuse an updated resource'),
        };
    if (record.metadata.identifier !== record.envelope.recordId)
        return {
            ok: false,
            diagnostic: diagnostic('ROLE_IDENTIFIER_MISMATCH', record.metadata.identifier, 'trusted identifier does not match the role operation record id'),
        };
    const operation = record.envelope.body;
    if (record.metadata.publisherName.trim().toLowerCase() !==
        operation.actorName.trim().toLowerCase())
        return {
            ok: false,
            diagnostic: diagnostic('ROLE_FORGED_ACTOR', record.metadata.identifier, 'embedded role actor does not match the QDN publisher'),
        };
    ...
    if (transaction.type !== 'ARBITRARY' ||
        ...
        return {
            ok: false,
            diagnostic: diagnostic('ROLE_TRANSACTION_MISMATCH', record.metadata.identifier, 'Core transaction evidence does not match the discovered role resource'),
        };
    if (transaction.name.trim().toLowerCase() !==
        record.metadata.publisherName.trim().toLowerCase())
        return {
            ok: false,
            diagnostic: diagnostic('ROLE_UNTRUSTED_PUBLISHER', record.metadata.identifier, 'Core transaction name does not match the discovered QDN publisher'),
        };
    if (transaction.creatorAddress.trim() !== operation.actorAddress.trim())
        return {
            ok: false,
            diagnostic: diagnostic('ROLE_PUBLISHER_WALLET_MISMATCH', record.metadata.identifier, 'Core transaction creator does not match the claimed actor wallet'),
        };
    const authorized = validateRoleMutationAgainstPriorState(operation, registry, checkpoint);
    if (authorized.ok === false)
        return {
            ok: false,
            diagnostic: diagnostic(authorized.code, record.metadata.identifier, authorized.detail),
        };
    return authorized;
};
export const reduceRoleOperations = (input) => {
    ...
    [...groups.entries()]
        ...
        if (distinct.size > 1) {
            const first = [...group].sort((left, right) => compareRoleOrder(left.metadata, right.metadata))[0];
            diagnostics.push(diagnostic('ROLE_OPERATION_CONFLICT', first.metadata.identifier, 'conflicting records reuse one publisher/identifier resource key'));
            return;
        }
    ...
    candidates
        ...
        if (bootstrapOrder &&
            ...
            diagnostics.push(diagnostic('ROLE_OPERATION_PREDATES_BOOTSTRAP', record.metadata.identifier, 'role operation is not later than the trusted bootstrap checkpoint'));
            return;
        }
        ...
        const resultingRole = body.action === 'assign' ? body.role : 'Member';
        if (body.action === 'assign' ||
            (body.action === 'revoke' && valid.targetRole === body.role))
            registry = normalizeRoleRegistry(replaceRole(registry, body.targetAddress, resultingRole));
        checkpoint = {
...
export const resolveRoleAt = (state, metadata) => {
