...
const escapeHtml = (value) => value
    ...
    .replace(/"/g, '&quot;')
    .replace(/'/g, '&#39;');
const replacePatternRecursively = (value, pattern, replacer, maxPasses = 6) => {
    let current = value;
...
const QDN_IMAGE_PLACEHOLDER_TAG_PATTERN = /\[imgqdnplaceholder\]([\s\S]*?)\[\/imgqdnplaceholder\]/gi;
const QDN_IMAGE_MISSING_TAG_PATTERN = /\[imgqdnmissing\]([\s\S]*?)\[\/imgqdnmissing\]/gi;
const QDN_VIDEO_TAG_PATTERN = /\[videoqdn\]([\s\S]*?)\[\/videoqdn\]/gi;
const QDN_LINK_PATTERN = /qdn:\/\/[^\s<]+/gi;
const TRAILING_PUNCTUATION_PATTERN = /[.,!?;:)\]}]+$/;
const decodeQdnTagPart = (value) => {
    try {
...
export const parseQdnImageTagPayload = (payload) => {
...
const splitLinkAndTrailingPunctuation = (value) => {
...
const linkifyQdnUris = (html) => {
    ...
    return segments
        ...
        return segment.replace(QDN_LINK_PATTERN, (rawLink) => {
...
const renderQdnImagePlaceholder = (payload) => {
...
export const extractQdnImageTags = (value) => {
    const found = [];
...
export const stripQdnImageTags = (value) => {
    return value.replace(QDN_IMAGE_TAG_PATTERN, '');
};
export const stripQdnVideoTags = (value) => {
    return value.replace(QDN_VIDEO_TAG_PATTERN, '');
};
export const applyWrapFormat = ({ value, selectionStart, selectionEnd, openTag, closeTag, placeholder = 'text', }) => {
...
export const applyListFormat = ({ value, selectionStart, selectionEnd, ordered, }) => {
    const before = value.slice(0, selectionStart);
    const selected = value.slice(selectionStart, selectionEnd);
    const after = value.slice(selectionEnd);
...
export const toRichTextHtml = (value) => {
    ...
    html = replacePatternRecursively(html, /\[h2\]([\s\S]*?)\[\/h2\]/gi, (_full, content) => `<h2 class="mt-4 mb-2 text-xl font-bold tracking-tight text-slate-900">${content}</h2>`);
    html = replacePatternRecursively(html, /\[h3\]([\s\S]*?)\[\/h3\]/gi, (_full, content) => `<h3 class="mt-3 mb-2 text-lg font-bold tracking-tight text-slate-900">${content}</h3>`);
    html = replacePatternRecursively(html, /\[icode\]([\s\S]*?)\[\/icode\]/gi, (_full, content) => `<code class="rounded-md border border-slate-300 bg-slate-100 px-1.5 py-0.5 font-mono text-[0.92em] text-slate-800">${content}</code>`);
    html = replacePatternRecursively(html, /\[b\]([\s\S]*?)\[\/b\]/gi, (_full, content) => `<strong>${content}</strong>`);
    html = replacePatternRecursively(html, /\[i\]([\s\S]*?)\[\/i\]/gi, (_full, content) => `<em>${content}</em>`);
    html = replacePatternRecursively(html, /\[u\]([\s\S]*?)\[\/u\]/gi, (_full, content) => `<u>${content}</u>`);
    html = replacePatternRecursively(html, /\[s\]([\s\S]*?)\[\/s\]/gi, (_full, content) => `<s>${content}</s>`);
    html = replacePatternRecursively(html, /\[quote\]([\s\S]*?)\[\/quote\]/gi, (_full, content) => `<blockquote class="my-2 border-l-2 border-slate-300 pl-3 text-slate-600">${content}</blockquote>`);
    html = replacePatternRecursively(html, /\[code\]([\s\S]*?)\[\/code\]/gi, (_full, content) => `<div class="my-4 overflow-hidden rounded-xl border border-slate-800 bg-slate-950 shadow-sm"><div class="flex items-center justify-between border-b border-slate-800 bg-slate-900/95 px-3 py-2"><span class="text-[11px] font-semibold uppercase tracking-[0.2em] text-cyan-300">Code</span><span class="text-[11px] text-slate-400">formatted block</span></div><pre class="overflow-x-auto px-4 py-3 text-[13px] leading-6 text-slate-100"><code class="font-mono">${content}</code></pre></div>`);
    html = replacePatternRecursively(html, /\[ul\]([\s\S]*?)\[\/ul\]/gi, (_full, content) => {
        const items = content.replace(/\[li\]([\s\S]*?)\[\/li\]/gi, '<li>$1</li>');
        return `<ul class="my-3 list-disc space-y-1 pl-5">${items}</ul>`;
    });
    html = replacePatternRecursively(html, /\[ol\]([\s\S]*?)\[\/ol\]/gi, (_full, content) => {
        const items = content.replace(/\[li\]([\s\S]*?)\[\/li\]/gi, '<li>$1</li>');
        return `<ol class="my-3 list-decimal space-y-1 pl-5">${items}</ol>`;
    });
    html = replacePatternRecursively(html, /\[color=(#[0-9a-fA-F]{6})\]([\s\S]*?)\[\/color\]/g, (_full, color, content) => `<span style="color:${color}">${content}</span>`);
    html = replacePatternRecursively(html, /\[img\]([\s\S]*?)\[\/img\]/gi, (_full, content) => {
    ...
    html = replacePatternRecursively(html, QDN_IMAGE_PLACEHOLDER_TAG_PATTERN, (_full, payload) => renderQdnImagePlaceholder(payload));
    html = replacePatternRecursively(html, QDN_IMAGE_MISSING_TAG_PATTERN, () => `<div class="my-3 max-w-xs rounded-lg border border-amber-200 bg-amber-50 p-3 text-sm text-amber-800">QDN image is not available yet.</div>`);
    html = replacePatternRecursively(html, QDN_IMAGE_TAG_PATTERN, (_full, payload) => renderQdnImagePlaceholder(payload));
    html = replacePatternRecursively(html, QDN_VIDEO_TAG_PATTERN, (_full, payload) => {
        const safePayload = String(payload ?? '').trim();
        if (!safePayload) {
            return '';
        }
