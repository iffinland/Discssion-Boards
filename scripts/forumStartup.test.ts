...
import {
  configureStartupDiagnosticsForTest,
  getStartupDiagnosticsReport,
  recordStartupEvent,
  resetStartupDiagnosticsForTest,
} from '../src/services/perf/startupDiagnostics.js';
import {
  preserveStartupDebugQuery,
  selectReadyDerivedCopies,
  StartupTimeoutError,
  withStartupTimeout,
} from '../src/services/perf/startupControl.js';
...
test('startup diagnostics are disabled by default', () => {
  ...
  assert(
    getStartupDiagnosticsReport().events.length === 0,
    'disabled diagnostics must not retain events'
  );
});
...
test('diagnostics redact secret-like detail and group duplicate requests', () => {
  ...
  assert(
    report.duplicateRequests[0]?.count === 2 &&
      report.duplicateRequests[0].cumulativeDurationMs === 20,
    'duplicate request summary is incorrect'
  );
});
...
test('request instrumentation records safe mocked bridge metadata', async () => {
  ...
  try {
    ...
    if (previous) scope.qdnRequest = previous;
    else Reflect.deleteProperty(scope, 'qdnRequest');
  }
  const requestEnd = getStartupDiagnosticsReport().events.find(
    (event) => event.name === 'REQUEST_END'
  );
...
test('normal, partial, empty, and failed results preserve their values', async () => {
  const normal = await withStartupTimeout(
    Promise.resolve({ completeness: 'complete', items: ['topic'] }),
    50,
    'normal'
  );
  const partial = await withStartupTimeout(
    Promise.resolve({ completeness: 'partial', items: ['topic'] }),
    50,
    'partial'
  );
  const empty = await withStartupTimeout(
    Promise.resolve({ completeness: 'complete', items: [] as string[] }),
    50,
    'empty'
  );
...
test('ready derived copies exclude unavailable duplicates', () => {
  ...
  assert(
    selected.candidates.length === 1 && selected.candidates[0]?.id === 'ready',
    'ready copy was not preferred'
  );
  assert(
    selected.skippedUnavailableCount === 1,
    'skipped copy was not counted'
  );
});
...
test('non-ready copies remain candidates when no ready copy exists', () => {
  ...
  assert(
    selected.skippedUnavailableCount === 0,
    'fallback must not claim skipped copies'
  );
});

test('debug query survives share and direct-route normalization', () => {
  assert(
    preserveStartupDebugQuery('?thread=t1&debugStartup=1', '?post=p1') ===
      '?post=p1&debugStartup=1',
    'thread redirect lost debug gate'
  );
  assert(
    preserveStartupDebugQuery('?debugStartup=1') === '?debugStartup=1',
    'direct route lost debug gate'
  );
  assert(
    preserveStartupDebugQuery('?thread=t1', '?post=p1') === '?post=p1',
    'normal routing gained debug UI'
  );
});

test('runtime startup keeps authoritative discovery foreground-only', async () => {
  const source = await readFile(
    'src/features/forum/hooks/useForumDataQuery.ts',
    'utf8'
  );
  const indexStart = source.indexOf('.loadTopicDirectoryIndex()');
  const authoritativeAwait = source.indexOf(
    'let remoteData = await structurePromise'
  );
  const fallbackAwait = source.indexOf(
    'const nextTopicDirectoryIndex = await withStartupTimeout'
  );
  assert(indexStart >= 0, 'background derived index is missing');
  assert(
    authoritativeAwait > indexStart,
    'authoritative discovery did not start'
  );
  assert(
    fallbackAwait > authoritativeAwait,
    'derived index is awaited before authoritative failure'
  );
  assert(
    source.includes("setStartupState('ready-partial', 'partial')"),
    'partial fallback state is missing'
  );
});
...
test('runtime startup supports guest reads, cancellation, stale guards, and retry', async () => {
  ...
  assert(
    source.includes("detail: 'guest-read-mode'"),
    'identity failure does not enter guest read mode'
  );
  assert(
    source.includes('if (!active) return') && source.includes('active = false'),
    'cancelled/stale work is not guarded'
  );
  assert(
    source.includes('setRetryGeneration((current) => current + 1)'),
    'retry cannot force a new generation'
  );
  assert(
    !source.includes(
      'await forumSearchIndexService.loadTopicDirectoryIndex();'
    ),
    'derived index still blocks startup'
  );
});
...
test('Home settings remain outside the forum readiness dependency graph', async () => {
  ...
  const provider = await readFile(
    'src/context/DisplaySettingsContext.tsx',
    'utf8'
  );
  assert(
    provider.includes('void loadHomeDisplaySettings'),
    'Home settings became blocking'
  );
  assert(
    root.indexOf('<DisplaySettingsProvider>') < root.indexOf('<ForumProvider>'),
    'provider composition unexpectedly changed'
  );
});
